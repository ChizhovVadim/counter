...
use crate::eval::pesto::{side_piece_sq_index, Score, Weights};

fn s(mg: i32, eg: i32) -> Score {
    return Score { mg: mg, eg: eg };
}

pub fn init_weights(w: &mut Weights) {
    w.bishop_pair = s(25, 25);

    let pawn = s(82, 94);
    let knight = s(337, 281);
    let bishop = s(365, 297);
    let rook = s(477, 512);
    let queen = s(1025, 936);
    let king = s(0, 0);

    for sq in 0..SQUARE_NB {
        let flip_sq = flip_square(sq);

        let index = side_piece_sq_index(SIDE_WHITE, PIECE_PAWN, sq);
        w.psq[index] = pawn;
        w.psq[index].mg += PAWN_MG[flip_sq];
        w.psq[index].eg += PAWN_EG[flip_sq];

        let index = side_piece_sq_index(SIDE_WHITE, PIECE_KNIGHT, sq);
        w.psq[index] = knight;
        w.psq[index].mg += KNIGHT_MG[flip_sq];
        w.psq[index].eg += KNIGHT_EG[flip_sq];

        let index = side_piece_sq_index(SIDE_WHITE, PIECE_BISHOP, sq);
        w.psq[index] = bishop;
        w.psq[index].mg += BISHOP_MG[flip_sq];
        w.psq[index].eg += BISHOP_EG[flip_sq];

        let index = side_piece_sq_index(SIDE_WHITE, PIECE_ROOK, sq);
        w.psq[index] = rook;
        w.psq[index].mg += ROOK_MG[flip_sq];
        w.psq[index].eg += ROOK_EG[flip_sq];

        let index = side_piece_sq_index(SIDE_WHITE, PIECE_QUEEN, sq);
        w.psq[index] = queen;
        w.psq[index].mg += QUEEN_MG[flip_sq];
        w.psq[index].eg += QUEEN_EG[flip_sq];

        let index = side_piece_sq_index(SIDE_WHITE, PIECE_KING, sq);
        w.psq[index] = king;
        w.psq[index].mg += KING_MG[flip_sq];
        w.psq[index].eg += KING_EG[flip_sq];
    }

    for piece in PIECE_PAWN..=PIECE_KING {
        for sq in 0..SQUARE_NB {
            let black_index = side_piece_sq_index(SIDE_BLACK, piece, flip_square(sq));
            let white_index = side_piece_sq_index(SIDE_WHITE, piece, sq);
            w.psq[black_index].mg = -w.psq[white_index].mg;
            w.psq[black_index].eg = -w.psq[white_index].eg;
        }
    }
}
...
static KNIGHT_MG: [i32; SQUARE_NB] = [
    // knight
...
static BISHOP_MG: [i32; SQUARE_NB] = [
    // bishop
...
static PAWN_EG: [i32; SQUARE_NB] = [
    //pawn
    0, 0, 0, 0, 0, 0, 0, 0, 178, 173, 158, 134, 147, 132, 165, 187, 94, 100, 85, 67, 56, 53, 82, 84,
    32, 24, 13, 5, -2, 4, 17, 17, 13, 9, -3, -7, -7, -8, 3, -1, 4, 7, -6, 1, 0, -5, -1, -8, 13, 8,
    8, 10, 13, 0, 2, -7, 0, 0, 0, 0, 0, 0, 0, 0,
];

static KNIGHT_EG: [i32; SQUARE_NB] = [
    // knight
...
static BISHOP_EG: [i32; SQUARE_NB] = [
    // bishop
...
static QUEEN_EG: [i32; SQUARE_NB] = [
    // queen
    -9, 22, 22, 27, 27, 19, 10, 20, -17, 20, 32, 41, 58, 25, 30, 0, -20, 6, 9, 49, 47, 35, 19, 9, 3,
    22, 24, 45, 57, 40, 57, 36, -18, 28, 19, 47, 31, 34, 39, 23, -16, -27, 15, 6, 9, 17, 10, 5,
    -22, -23, -30, -16, -16, -23, -36, -32, -33, -28, -22, -43, -5, -32, -20, -41,
];

static KING_EG: [i32; SQUARE_NB] = [
    // king
